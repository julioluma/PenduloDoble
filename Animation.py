...
for idx in [5, 10, 15]:  # Cambia el rango según las trayectorias que tengas

    tray = np.loadtxt(f"trayectorias_txt/trayectoria_{idx}.txt")
    ...
    x1_1, y1_1, x2_1, y2_1 = extraer_posiciones(tray)
    ...
    ax.set_ylim(-2.2, 1.2)
    ax.set_aspect('equal')
    ax.set_title("Animación del péndulo doble")

    line, = ax.plot([], [], 'o-', color='royalblue', lw=2, label=f'Energia={idx}')
    ax.legend()

    def update(i):
        line.set_data([0, x1_1[i], x2_1[i]], [0, y1_1[i], y2_1[i]])
        return (line,)
    ...
    if guardar:
        ...
        nombre_salida = f"animations/animacion_{idx}.mp4"
